...
/**
 * Props for the UserRoleSettingsPanel component
 */
...
const UserRoleSettingsPanel: React.FC<UserRoleSettingsPanelProps> = () => {
  ...
  const groupedRoles = useMemo(() => {
    const groups: Record<string, typeof userRoles> = {};

    userRoles
      .filter(role => showHidden ? role.hidden : !role.hidden)
      .filter(role =>
    ...
    return groups;
  }, [userRoles, searchTerm, showHidden]);
  ...
  const toggleAllCategories = () => {
    ...
    Object.keys(groupedRoles).forEach(category => {
    ...
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <User2 className="inline-flex" size="16"/> User Roles <Badge>
          <span title="Visible count">{userRoles.filter(role => !role.hidden).length}</span> <span className="ml-1 mr-1">/</span>
        <span title="Total count">{userRoles.length}</span></Badge>
        </h3>
