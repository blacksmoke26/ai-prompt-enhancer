...
>
  <div className="space-y-4">
    <p>Are you sure you want to update your public profile?</p>
...
>
  <p className="text-sm">Please type "DELETE" to confirm.</p>
</Popover>
```

#### 3. Custom Trigger & Size
```tsx
<Popover 
  width="w-[500px]"
  trigger={<Button className="w-full">Open Large Popover</Button>}
  showFooter={false}
  showHeader={false}
>
  <MyComplexForm />
...
Here is a comprehensive showcase of examples demonstrating the `Popover` component in various real-world scenarios. This includes forms, scrollable lists, text previews, complex filters, and destructive actions.

### The Showcase Component

You can copy this entire block into a file (e.g., `PopoverExamples.tsx`) to see everything running at once.
...
import {Popover} from './Popover'; // Import the component created previously
  ...
  return (
    <div className="space-y-2">
      ...
      <Popover
        ...
        description="This action cannot be undone."
        triggerText="Delete Project" // Assuming your button supports text, otherwise use trigger prop
...
### Key Highlights of these Examples

1.  **Quick Form Example**:
    *   Uses `onConfirm` with a simulated async delay.
    *   Demonstrates `showReset` and custom icons (`resetIcon`).
    *   Shows how to wrap form inputs (`select`, `input`) inside the body.

2.  **Scrollable List Example**:
    *   Uses `maxHeight="h-64"` combined with a standard `overflow-y-auto` class (handled internally by the component when `maxHeight` is passed).
    *   Demonstrates a search input inside a popover, a very common pattern.
    *   Uses `width="w-80"` to give it a narrow list look.

3.  **Large Text Example**:
    *   Uses `width="w-[500px]"` to make it look like a dialog or modal.
    *   Uses `prose` (Typography) classes to show how text renders inside the popover.
    *   Uses `align="start"` to anchor it differently than the default `end`.

4.  **Destructive Example**:
    *   Sets `variant="destructive"`. This automatically changes border colors, icon colors, and button colors.
    *   Overrides `trigger` with a custom destructive button.
    *   Uses `closeOnConfirm={true}`.

5.  **Advanced Filter Example**:
    *   Uses `radius="xl"` and `shadow="2xl"` for a "floating card" aesthetic.
    *   Complex layout with multiple sections inside the body.

6.  **Profile Menu Example**:
    *   Disables `showHeader` and `showFooter` completely.
    *   Uses a completely custom `trigger` (an avatar button).
    *   Renders a simple list of buttons inside the body (`children`), acting like a Dropdown Menu.

7.  **Glassmorphism Example**:
    *   Uses `backdrop={true}` (assuming your Tailwind config supports standard backdrop blur utilities, which the component adds `backdrop-blur-md` for).
    *   Uses `variant="warning"` to demonstrate color flexibility.
