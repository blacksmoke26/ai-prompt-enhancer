...
export const componentsMaps: Record<string, { title: string; component: Function | null; }> = {
  ...
  minimalOutput: {title: '', component: null},
  ...
  technicalJargonLevel: {title: '', component: null},
  ...
  thinkingPattern: {title: '', component: null},
  ...
  geographicScope: {title: '', component: null},

  // General Structural Details
  detailLevel: {title: '', component: null},
...
export const VISIBILITY_PROFILES: Record<string, { label: string; desc: string; config: VisibleComponents }> = {
  ...
  creative: {
    ...
    desc: 'Focus on tone, audience, and creativity',
    config: {
      ...PROFILE_ALL_FALSE,
      temperature: true,
      maxTokens: true,
  ...
  coder: {
    ...
    desc: 'Technical precision and model parameters',
    config: {
      ...PROFILE_ALL_FALSE,
      temperature: true,
      maxTokens: true,
  ...
  analyst: {
    label: 'Data Analyst',
    desc: 'Structure, JSON output and token limits',
    config: {
      ...PROFILE_ALL_FALSE,
      maxTokens: true,
      format: true,
  ...
  chat: {
    ...
    desc: 'Persona, tone and instructions',
    config: {
      ...PROFILE_ALL_FALSE,
      temperature: true,
      maxTokens: true,
      ...
      language: true,
      culturalContext: true,
      memory: true, // Assuming memory might be relevant for conversation, though not in 'power' list, sticking to available keys
      offTheRecord: true,
    } as VisibleComponents,
  },
  educator: {
    label: 'Educator / Tutor',
    desc: 'Clear explanations, audience adaptation',
    config: {
      ...PROFILE_ALL_FALSE,
      temperature: true,
      maxTokens: true,
      ...
      reasoningDepth: true,
      explanationStyle: true, // mapped to customInstructions/tone implicitly, but using available keys
      culturalContext: true,
  ...
  business: {
    ...
    desc: 'Formal tone, structured output',
    config: {
      ...PROFILE_ALL_FALSE,
      temperature: true,
      maxTokens: true,
  ...
  researcher: {
    label: 'Researcher',
    desc: 'Detailed analysis with markdown formatting',
    config: {
      ...PROFILE_ALL_FALSE,
      temperature: true,
      maxTokens: true,
      ...
      includeMetadata: true,
      language: true,
      citationStyle: true, // implied by format/strictness, using available keys
...
export const COMPONENT_CATEGORIES: Record<string, { label: string; icon: any }> = {
  core: { label: 'Core Controls', icon: Sliders },
  style: { label: 'Style & Tone', icon: Sparkles },
  model: { label: 'Model Parameters', icon: Cpu },
  advanced: { label: 'Advanced', icon: ShieldAlert },
};
