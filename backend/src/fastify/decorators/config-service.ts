 ...
 * This plugin asynchronously initializes a `ConfigManager` instance and attaches it to the Fastify instance as a decorator.
 *
 ...
 * - The `ConfigManager` class should be defined elsewhere and must be fully initialized before use.
 * - Ensure that `ConfigManager` is properly implemented with methods like `get()` and `set()` for configuration access.
...
export default async (app: FastifyInstance): Promise<AppDecorator> => {
  const manager = new ConfigService();
  return {
    name: 'configService',
    value: manager,
  };
