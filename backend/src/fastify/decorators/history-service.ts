 ...
 * This plugin asynchronously initializes a `HistoryManager` instance and attaches it to the Fastify instance as a decorator.
 *
 ...
 * - The `HistoryManager` class should be defined elsewhere and must be fully initialized before use.
 * - Ensure that `HistoryManager` is properly implemented with methods like `get()` and `set()` for configuration access.
...
export default async (app: FastifyInstance): Promise<AppDecorator> => {
  const manager = new HistoryService();
