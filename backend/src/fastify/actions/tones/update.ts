...
// db
import { Tone } from '~/database/models';
...
export default (fastify: FastifyInstance) => {
  /**
   * Updates tone identified by the provided key.
  ...
  fastify.put<{
    ...
    if (!record) {
      ErrorHelper.throwWithStatus('Tone not found', 404);
    }
    ...
    try {
      ...
      fastify.log.error('Failed to update tone:', error);
      ErrorHelper.throwWithStatus('Failed to update tone');
    }
  });
};
